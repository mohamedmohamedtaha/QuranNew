...
class ReaderViewModel @Inject constructor(): ViewModel(){
    ...
    fun getDataBySearch(context: Context,search:String) {
        val images = ArrayList<ImageModel>()
        images.add(
            ImageModel(
                context.getString(R.string.dokaly_mohamed),
        ...
        val searchImage = arrayListOf<ImageModel>()
        for (position in images.indices){
            if (!images.isNullOrEmpty())
                if (images[position].nameShekh!!.contains(search)){
                    searchImage.add(images[position])
                }
        }
        _sound.value = searchImage
    }
